 ...
 * @date 2020/10/24
 */
public class SelectIncludesProvider extends MapperTemplate {

    public SelectIncludesProvider(Class<?> mapperClass, MapperHelper mapperHelper) {
    ...
    public String selectByProperties(MappedStatement ms) {

        String propertyHelper = SelectIncludesProvider.class.getName();
        Class<?> entityClass = this.getEntityClass(ms);
        this.setResultType(ms, entityClass);
        StringBuilder sql = new StringBuilder();
        sql.append("SELECT ");
        String entityClassName = entityClass.getName();
        sql.append("<foreach collection=\"fields\" item=\"item\" open=\"\" separator=\",\" close=\"\">");
