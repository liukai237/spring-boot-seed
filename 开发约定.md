...
* 业务与SQL分离，建议少用~~Criteria~~风格的高级语法糖。
...
* `Pageable`一般和`PageData`成对出现，通过`QueryBuilder`工具类能够轻松获取到包含分页排序参数Entity实体或者Example（Condition）对象。
> 多表或者复杂的分页排序请在service层重新定义DTO继承Pageable。
...
// Controller层
public Resp<PageData<UserDetailDto>> queryUsers(@RequestBody PageQuery<UserQueryParam> req) {
    return ok(userService.findByCondition(QueryBuilder.init(req).build(User.class)));
}

// Service层
@Transactional(readOnly = true)
public PageData<UserDetailDto> findByCondition(User entity) {
    return new PageData<>(userMapper.selectPage(entity), UserConverter.INSTANCE::toDto);
}
```
> 注意：
...
* 与Entity对应的Service可以继承BaseService，以实现基本的CRUD操作。
