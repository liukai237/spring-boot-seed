...
// Controller层
public Resp<PageData<UserDetailDto>> queryWithPage(@RequestBody PageQuery<UserQuery> req) {
    return ok(userService.findByCondition(req.asCondition(User.class)));
}
...
### 二级缓存
在Mapper上增加`@CacheNamespace`注解即可实现二级缓存，但是只针对通用方法有效，如果通用方法与XML混合使用，需要做如下配置：
#### XML中增加<cache/>配置：
```xml
