...
* `PageQuery`一般和`PageData`成对出现，`toQuery()`方法能够获取到包含分页排序参数Entity实体对象。
...
public Resp<PageData<UserDetailDto>> queryUsers(@RequestBody PageQuery<UserQueryParam> req) {
    return ok(userService.findByCondition(req.toQuery(User.class)));
}
...
```
> 注意：
> * 查询参数中的属性应该与Entity保持一致，否则过滤条件将不生效。
...
> 通过MBG一次性生成泛型化的Service或者通用Service，尽量不要再次生成。
